...
class MarvelRepository @Inject constructor(
	...
	fun cacheSearchKeyword(keyword: SearchKeyword) {
		searchDao.insertKeyword(keyword.toKeywordEntity()).subscribeOn(Schedulers.io()).subscribe()
	}

	//fun getKeywords() = searchDao.getAllKeywords().map { it.map { it.toKeyword() } }

	/*private fun SearchKeywordEntity.toKeyword(): SearchKeyword {
		return dataMapper.keywordEntityToKeywordMapper.map(this)
	}*/

	private fun SearchKeyword.toKeywordEntity(): SearchKeywordEntity {
		return dataMapper.keywordToKeywordEntityMapper.map(this)
	}
