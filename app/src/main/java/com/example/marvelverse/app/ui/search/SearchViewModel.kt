...
class SearchViewModel @Inject constructor(private val repository: MarvelRepository) :
	...
	private val _characterList =
		MutableLiveData<DataState<com.example.marvelverse.domain.entities.Character>>()
	...
	init {
		...
		_searchResult.addSource(_comicList) { comics ->
			_searchResult.value = SearchItems(
				comics,
				_characterList.value ?: DataState.Empty,
				_eventList.value ?: DataState.Empty
			)
		}

		_searchResult.addSource(_characterList) { characters ->
			_searchResult.value = SearchItems(
				_comicList.value ?: DataState.Empty,
				characters,
				_eventList.value ?: DataState.Empty
			)
		}
	...
	fun cacheKeyword(keyword: SearchKeyword) {
		repository.cacheSearchKeyword(keyword)
	}
